<template>
  <div class="tree-contanier" id="tree" ref="treeContainer">
    <h2 style="margin-top: 5px;color: #124880;">|&emsp;MBOM（制造BOM）结构树</h2>
    <el-row>
      <el-col :span="24" class="chart chart_left">
        <div id="treeChart" style="height: 450px">MBOM（制造BOM）结构树加载失败</div>
      </el-col>
    </el-row>
    <el-divider></el-divider>
    <h2 style="float: left; margin-top:5px;color: #124880;">|&emsp;MBOM（制造BOM）数据表格</h2>
...
<!--              </el-table-column>-->
      ...
      <div slot="footer" class="dialog-footer" style="margin-top: 50px">
        <el-upload class="upload-demo inline-block" ref="upload" :multiple="true" style="float: left;margin-left: 62%"
                   ...
                   action="/BOMFile/dataFile/uploadMBomFile" :on-preview="handlePreview"
    ...
    <el-dialog
      ...
      <div>
        ...
        <el-button type="primary" size="medium" @click="handleMallCatergoryChange">查看关联属性信息</el-button>
      </div>
    ...
    <el-dialog
      ...
      <div>
        ...
        <el-button type="primary" size="medium" @click="handleStCatergoryChange">查看关联属性信息</el-button>
      </div>
...
<!--			<el-button @click="selectStFile" type="primary" size="small">查看节点详细信息请下载源文件</el-button>-->
        <!-- <el-button type="success" @click="">查看节点详细信息请下载源文件</el-button>  -->
		    ...
		    <el-button type="primary" @click="connectStBom" size="small">关联完毕</el-button>
		  </span>
      </div>
    </el-dialog>
    <!-- 选择关联节点信息和数据结束 -->
    ...
    <el-dialog title="关联关系添加页面" :visible.sync="visitOutBOM" width="85%" :before-close="handleClose">
      ...
      <!-- prop="children" :label-width="formLabelWidth" :label-positon="labelPositon"> -->
      <div>
        <el-cascader ref="cascaderMallCatergoryOut" size="medium" :key="isResouceShow" v-model="temp.children5"
    ...
    export default {
        ...
        data() {
            return {
                ...
                alignDir: 'center',
                textMap: {
                    update: '编辑制造BOM(MBOM)节点信息',
                    create: '添加制造BOM(MBOM)节点信息',
                },
                dialogStatus: '',
        ...
        methods: {
            ...
            loadFileForm(fileValue) {
                this.getRequest("/BOMFile/dataFile/findMBomFileByPage?page=" + this.currentPage +
                    "&size=" + this.pageSize +
            ...
            downLoadFile(index, row) {
                ...
                this.apiDownLoad("/BOMFile/dataFile/downloadMBomFileByName?fileId=" + fileId + "&fileName=" + fileName, {responseType: 'blob'}).then(
                    resp => {
            ...
            deleteFile(index, row) {
                ...
                // 	});
                // } else {
                this.postRequest("/BOMFile/dataFile/deleteMBomFile?fileId=" + id).then(resp => {
                    if (resp && resp.status == 200) {
                        this.loadFileForm();
                    } else {
                        this.$message({
                ...
                this.$refs['dataForm'].validate((valid) => {
                    if (valid) {
                        if (this.sonStatus == false) {
                            ...
                            // if (this.casArr[this.casArr.length - 1].includes('B')) {
                            //     this.temp.value =
                            //         String(this.casArr[this.casArr.length - 1]) + '-' +
                            //         String(arr.length);
                            // } else {
                            //     this.temp.value =
                            //         String('B' + this.casArr[this.casArr.length - 1]) + '-' +
                            //         String(arr.length);
                            // }
                            if (this.casArr[this.casArr.length - 1].includes('A')) {
                              this.temp.value =
                                String('D' +String(this.casArr[this.casArr.length - 1]).slice(1)) + '-' +
                                String(arr.length);
                              ...
                              this.temp.value =
                                String('D' + this.casArr[this.casArr.length - 1]) + '-' +
                                String(arr.length);
                            }
            ...
            saveUpdateDataJSON() {
                ...
                this.$http({
                    ...
                    if (resp && resp.status == 200) {
                        ...
                        ;
                        // lwy  调用新增发布方法
                        this.publishAddNode("mbom_infomation",this.conStBomId)
                        // lwy 新增的节点信息 发布结束
                        this.$message({
                            type: 'success',
            ...
            publishAddNode(tableName,nodeValue){
                console.log("发布方法拿到了nodeValue",nodeValue)
                if(nodeValue == ''){
                    this.$message({
                        type: 'warning',
            ...
            handleUpdate(index, row) {
                ...
                //获取关联节点
                this.getRequest("/mbom/data/selectConnectInfo?selectId=" + row.value).then(resp => {
                    if (resp && resp.status == 200) {
                        console.log(resp);
            ...
            sentConnectInfo() {
                ...
                this.connectStInfoText.connectInfoListText = JSON.stringify(fileURLStore.getItem("connectInfoList")).toString();
                console.log("关联关系信息是");
                ...
                this.$http({
                    method: 'post',
                    url: "/mbom/data/saveStConnectData",
                    ...
                    if (resp && resp.status == 200) {
                        this.tableDataRetrun = resp.data.connectInfoList;
                        console.log("后端返回的数据为---------" + this.connectInfoList);
                        // this.getTableDataLate();
                        this.$message({
                            type: 'success',
            ...
            uploadFile(row) {
                ...
                this.uploadFileRequest("/BOMFile/dataFile/uploadMBomFile", uploadForm).then(resp => {
                    this.fileList = [];
            ...
            selectFile() {
                if (this.dfBomArrayElement == '' || this.dfBomArrayElement == null) {
                    ...
                    this.getRequest("/BOMFile/dataFile/selectMBomFile?dfBomNodeName=" + dfNodeName).then(resp => {
                        if (resp && resp.status == 200) {
            ...
            selectStFile() {
                if (this.stBomArrayElement == '' || this.stBomArrayElement == null) {
                    ...
                    this.getRequest("/BOMFile/dataFile/selectMBomFile?stBomNodeName=" + stNodeName).then(resp => {
                        if (resp && resp.status == 200) {
                            if (resp.data.fileName != null || this.fileName != '') {
                                this.fileName = resp.data.fileName;
                                this.downLoadStFile();
            ...
            downLoadStFile() {
                ...
                this.apiDownLoad("/BOMFile/dataFile/downloadMBomFileByName?fileName=" + this.fileName).then(
                    resp => {
                        var blob = new Blob([resp], {
                            type: "application/octet-stream"
                        });
        ...
        */
            ...
            sendrequestChange(row) {
                ...
                var tablename = 'mbom_infomation';
                var Associationtablename = 'mbom_connect_info';
            ...
            associationChange(row) {
                var submitintracoding = row.value;
                var tablename = 'mbom_connect_info';
                ...
                this.getRequest("/bomTree/associationDelete?submitNodeintracoding=" + submitNodeintracoding + "&submitintracoding=" + submitintracoding + "&tablename=" + tablename)
                    .then(resp => {
                        if (resp.data.deleteNum > 0 && resp.status == 200) {
                            ...
                            this.getRequest("/mbom/data/selectConnectInfo?selectId=" + row.connectId).then(resp => {
                                if (resp && resp.status == 200) {
                                    console.log(resp);
                                    this.connectInfoTable = resp.data.connectInfos;
                                    this.$message({
