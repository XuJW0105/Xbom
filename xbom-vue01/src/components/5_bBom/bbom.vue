<template>
  <div class="tree-contanier" id="tree" ref="treeContainer">
    <h2 style="margin-top: 5px;color: #124880;">|&emsp;BBOM（装机BOM）结构树</h2>
    <el-row>
      <el-col :span="24" class="chart chart_left">
        <div id="treeChart" style="height: 450px">BBOM（装机BOM）结构树加载失败</div>
      </el-col>
    </el-row>
    <el-divider></el-divider>
    <h2 style="float: left; margin-top:5px;color: #124880;">|&emsp;BBOM（装机BOM）数据表格</h2>
...
<!--              </el-table-column>-->
      ...
      <div slot="footer" class="dialog-footer" style="margin-top: 50px">
        <el-upload class="upload-demo inline-block" ref="upload" :multiple="true" style="float: left;margin-left: 62%"
                   ...
                   action="/BOMFile/dataFile/uploadBBomFile" :on-preview="handlePreview"
    ...
    <el-dialog title="关联关系添加页面" :visible.sync="visitOutBOM" width="85%" :before-close="handleClose">
      ...
      <div>
        <el-cascader ref="cascaderMallCatergoryOut" size="medium" :key="isResouceShow" v-model="temp.children6"
    ...
    export default {
        ...
        data() {
            return {
                ...
                alignDir: 'center',
                textMap: {
                    update: '编辑装机BOM(BBOM)节点信息',
                    create: '添加装机BOM(BBOM)节点信息',
                },
                dialogStatus: '',
        ...
        methods: {
            ...
            loadFileForm(fileValue) {
                this.getRequest("/BOMFile/dataFile/findBBomFileByPage?page=" + this.currentPage +
                    "&size=" + this.pageSize +
            ...
            downLoadFile(index, row) {
                ...
                this.apiDownLoad("/BOMFile/dataFile/downloadBBomFileByName?fileId=" + fileId + "&fileName=" + fileName, {responseType: 'blob'}).then(
                    resp => {
            ...
            deleteFile(index, row) {
                ...
                this.postRequest("/BOMFile/dataFile/deleteBBomFile?fileId=" + id).then(resp => {
                    if (resp && resp.status == 200) {
                        this.loadFileForm();
                    } else {
                        this.$message({
                    ...
                    if (valid) {
                        if (this.sonStatus == false) {
                            ...
                            if (this.casArr[this.casArr.length - 1].includes('A')) {
                              this.temp.value =
                                String('E' +String(this.casArr[this.casArr.length - 1]).slice(1)) + '-' +
                                String(arr.length);
                              ...
                              this.temp.value =
                                String('E' + this.casArr[this.casArr.length - 1]) + '-' +
                                String(arr.length);
                            }
            ...
            saveUpdateDataJSON() {
                ...
                this.$http({
                    ...
                    if (resp && resp.status == 200) {
                        ...
                        ;
                        // lwy  调用新增发布方法
                        this.publishAddNode("bbom_infomation",this.conStBomId)
                        // lwy 新增的节点信息 发布结束
                        this.$message({
                            type: 'success',
            ...
            handleUpdate(index, row) {
                ...
                //获取关联节点
                this.getRequest("/bbom/data/selectConnectInfo?selectId=" + row.value).then(resp => {
                    if (resp && resp.status == 200) {
                        console.log(resp);
            ...
            sentConnectInfo() {
                ...
                this.$http({
                    method: 'post',
                    url: "/bbom/data/saveStConnectData",
            ...
            uploadFile(row) {
                ...
                this.uploadFileRequest("/BOMFile/dataFile/uploadBBomFile", uploadForm).then(resp => {
                    this.fileList = [];
            ...
            selectFile() {
                if (this.dfBomArrayElement == '' || this.dfBomArrayElement == null) {
                    ...
                    this.getRequest("/BOMFile/dataFile/selectBBomFile?dfBomNodeName=" + dfNodeName).then(resp => {
                        if (resp && resp.status == 200) {
            ...
            selectStFile() {
                if (this.stBomArrayElement == '' || this.stBomArrayElement == null) {
                    ...
                    this.getRequest("/BOMFile/dataFile/selectBBomFile?stBomNodeName=" + stNodeName).then(resp => {
                        if (resp && resp.status == 200) {
                            if (resp.data.fileName != null || this.fileName != '') {
                                this.fileName = resp.data.fileName;
                                this.downLoadStFile();
            ...
            downLoadStFile() {
                ...
                this.apiDownLoad("/BOMFile/dataFile/downloadBBomFileByName?fileName=" + this.fileName).then(
                    resp => {
                        var blob = new Blob([resp], {
                            type: "application/octet-stream"
                        });
        ...
        */
            ...
            sendrequestChange(row) {
                ...
                var tablename = 'bbom_infomation';
                var Associationtablename = 'bbom_connect_info';
            ...
            associationChange(row) {
                var submitintracoding = row.value;
                var tablename = 'bbom_connect_info';
                ...
                this.getRequest("/bomTree/associationDelete?submitNodeintracoding=" + submitNodeintracoding + "&submitintracoding=" + submitintracoding + "&tablename=" + tablename)
                    .then(resp => {
                        if (resp.data.deleteNum > 0 && resp.status == 200) {
                            ...
                            this.getRequest("/bbom/data/selectConnectInfo?selectId=" + row.connectId).then(resp => {
                                if (resp && resp.status == 200) {
                                    console.log(resp);
                                    this.connectInfoTable = resp.data.connectInfos;
                                    this.$message({
